  ...
  const rerender = () => {
    ...
    working.cards.forEach((card, index) => {
      ...
      const createFaceControls = (faceKey, labelText) => {
        ...
        faceHeader.className = 'card-face-header';

        const faceTitle = document.createElement('h4');
        faceTitle.className = 'card-face-title';
        faceTitle.textContent = labelText;

        const colorInput = document.createElement('input');
        colorInput.type = 'color';
        colorInput.className = 'card-face-color';
        colorInput.setAttribute('aria-label', `${labelText} color`);
        colorInput.setAttribute('title', `${labelText} color`);
        const colorValue = card[`${faceKey}Color`];
        colorInput.value =
          typeof colorValue === 'string' && colorValue.trim() !== ''
            ? colorValue
            : faceKey === 'front'
            ? DEFAULT_FRONT_COLORS[index % DEFAULT_FRONT_COLORS.length]
            : DEFAULT_BACK_COLOR;
        ...
        if (locked) {
          ...
          faceContainer.classList.add('card-face-controls--locked');
        }

        faceHeader.append(faceTitle, colorInput);
        ...
        imageField.append(imageLabel, imageControls);
        imageField.append(preview);

        faceContainer.append(faceHeader, imageField);
